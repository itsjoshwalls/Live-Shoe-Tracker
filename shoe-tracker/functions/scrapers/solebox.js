...
export async function fetchList() {
  try {
    const res = await safeFetch("https://www.solebox.com/api/products");
    return res.products || res.items || res.results || res.objects || [];
  } catch (err) {
    console.error("solebox fetch failed:", err.message);
    return [];
  }
}
...
export function normalize(p) {
  return {
    id: "solebox-" + (p.id || p.handle || Date.now()),
    retailerId: "solebox",
    retailerName: "Solebox",
    ...
    url: "https://www.solebox.com/api/products/products/" + (p.handle || p.slug || p.id || ""),
    region: "US"
  };
}
