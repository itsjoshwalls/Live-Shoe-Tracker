...
export async function fetchList() {
  try {
    const res = await safeFetch("https://www.baitme.com/collections/footwear.json");
    return res.products || res.items || res.results || res.objects || [];
  } catch (err) {
    console.error("bait fetch failed:", err.message);
    return [];
  }
}
...
export function normalize(p) {
  return {
    id: "bait-" + (p.id || p.handle || Date.now()),
    retailerId: "bait",
    retailerName: "BAIT",
    ...
    url: "https://www.baitme.com/collections/footwear.json/products/" + (p.handle || p.slug || p.id || ""),
    region: "US"
  };
}
