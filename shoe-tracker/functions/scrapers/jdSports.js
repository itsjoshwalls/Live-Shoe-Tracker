...
export async function fetchList() {
  try {
    const res = await safeFetch("https://api.jdsports.com/products");
    return res.products || res.items || res.results || res.objects || [];
  } catch (err) {
    console.error("jdSports fetch failed:", err.message);
    return [];
  }
}
...
export function normalize(p) {
  return {
    id: "jdSports-" + (p.id || p.handle || Date.now()),
    retailerId: "jdSports",
    retailerName: "JD Sports",
    ...
    url: "https://api.jdsports.com/products/products/" + (p.handle || p.slug || p.id || ""),
    region: "US"
  };
}
