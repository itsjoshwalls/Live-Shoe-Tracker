...
export async function fetchList() {
  try {
    const res = await safeFetch("https://www.asphaltgold.com/collections/footwear.json");
    return res.products || res.items || res.results || res.objects || [];
  } catch (err) {
    console.error("asphaltgold fetch failed:", err.message);
    return [];
  }
}
...
export function normalize(p) {
  return {
    id: "asphaltgold-" + (p.id || p.handle || Date.now()),
    retailerId: "asphaltgold",
    retailerName: "Asphaltgold",
    ...
    url: "https://www.asphaltgold.com/collections/footwear.json/products/" + (p.handle || p.slug || p.id || ""),
    region: "US"
  };
}
