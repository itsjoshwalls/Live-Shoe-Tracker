...
export async function fetchList() {
  try {
    const res = await safeFetch("https://www.adidas.com/api/products");
    return res.products || res.items || res.results || res.objects || [];
  } catch (err) {
    console.error("adidas fetch failed:", err.message);
    return [];
  }
}
...
export function normalize(p) {
  return {
    id: "adidas-" + (p.id || p.handle || Date.now()),
    retailerId: "adidas",
    retailerName: "Adidas",
    ...
    url: "https://www.adidas.com/api/products/products/" + (p.handle || p.slug || p.id || ""),
    region: "US"
  };
}
