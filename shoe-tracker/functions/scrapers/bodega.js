...
export async function fetchList() {
  try {
    const res = await safeFetch("https://bdgastore.com/collections/footwear.json");
    return res.products || res.items || res.results || res.objects || [];
  } catch (err) {
    console.error("bodega fetch failed:", err.message);
    return [];
  }
}
...
export function normalize(p) {
  return {
    id: "bodega-" + (p.id || p.handle || Date.now()),
    retailerId: "bodega",
    retailerName: "Bodega",
    ...
    url: "https://bdgastore.com/collections/footwear.json/products/" + (p.handle || p.slug || p.id || ""),
    region: "US"
  };
}
