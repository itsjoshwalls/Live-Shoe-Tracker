...
export async function fetchList() {
  try {
    const res = await safeFetch("https://www.sneakersnstuff.com/en/537/sneakers");
    return res.products || res.items || res.results || res.objects || [];
  } catch (err) {
    console.error("sneakersnstuff fetch failed:", err.message);
    return [];
  }
}
...
export function normalize(p) {
  return {
    id: "sneakersnstuff-" + (p.id || p.handle || Date.now()),
    retailerId: "sneakersnstuff",
    retailerName: "Sneakersnstuff",
    ...
    url: "https://www.sneakersnstuff.com/en/537/sneakers/products/" + (p.handle || p.slug || p.id || ""),
    region: "US"
  };
}
