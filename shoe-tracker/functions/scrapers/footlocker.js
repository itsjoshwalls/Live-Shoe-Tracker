...
export async function fetchList() {
  try {
    const res = await safeFetch("https://www.footlocker.com/api/products");
    return res.products || res.items || res.results || res.objects || [];
  } catch (err) {
    console.error("footlocker fetch failed:", err.message);
    return [];
  }
}
...
export function normalize(p) {
  return {
    id: "footlocker-" + (p.id || p.handle || Date.now()),
    retailerId: "footlocker",
    ...
    url: "https://www.footlocker.com/api/products/products/" + (p.handle || p.slug || p.id || ""),
    region: "US"
  };
}
