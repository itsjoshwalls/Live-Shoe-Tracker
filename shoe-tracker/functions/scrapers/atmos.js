...
export async function fetchList() {
  try {
    const res = await safeFetch("https://www.atmosusa.com/collections/footwear.json");
    return res.products || res.items || res.results || res.objects || [];
  } catch (err) {
    console.error("atmos fetch failed:", err.message);
    return [];
  }
}
...
export function normalize(p) {
  return {
    id: "atmos-" + (p.id || p.handle || Date.now()),
    retailerId: "atmos",
    retailerName: "atmos",
    ...
    url: "https://www.atmosusa.com/collections/footwear.json/products/" + (p.handle || p.slug || p.id || ""),
    region: "US"
  };
}
