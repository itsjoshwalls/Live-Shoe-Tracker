...
export async function fetchList() {
  try {
    const res = await safeFetch("https://www.size.co.uk/api/products");
    return res.products || res.items || res.results || res.objects || [];
  } catch (err) {
    console.error("sizeOfficial fetch failed:", err.message);
    return [];
  }
}
...
export function normalize(p) {
  return {
    id: "sizeOfficial-" + (p.id || p.handle || Date.now()),
    retailerId: "sizeOfficial",
    retailerName: "size?",
    ...
    url: "https://www.size.co.uk/api/products/products/" + (p.handle || p.slug || p.id || ""),
    region: "US"
  };
}
