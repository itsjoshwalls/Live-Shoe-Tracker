...
export async function fetchList() {
  try {
    const res = await safeFetch("https://www.hanon-shop.com/collections/footwear.json");
    return res.products || res.items || res.results || res.objects || [];
  } catch (err) {
    console.error("hanon fetch failed:", err.message);
    return [];
  }
}
...
export function normalize(p) {
  return {
    id: "hanon-" + (p.id || p.handle || Date.now()),
    retailerId: "hanon",
    retailerName: "Hanon",
    ...
    url: "https://www.hanon-shop.com/collections/footwear.json/products/" + (p.handle || p.slug || p.id || ""),
    region: "US"
  };
}
