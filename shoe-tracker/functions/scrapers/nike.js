...
export async function fetchList() {
  try {
    const res = await safeFetch("https://api.nike.com/product_feed/rollup_threads/v2");
    return res.products || res.items || res.results || res.objects || [];
  } catch (err) {
    console.error("nike fetch failed:", err.message);
    return [];
  }
}
...
export function normalize(p) {
  return {
    id: "nike-" + (p.id || p.handle || Date.now()),
    retailerId: "nike",
    retailerName: "Nike",
    ...
    url: "https://api.nike.com/product_feed/rollup_threads/v2/products/" + (p.handle || p.slug || p.id || ""),
    region: "US"
  };
}
