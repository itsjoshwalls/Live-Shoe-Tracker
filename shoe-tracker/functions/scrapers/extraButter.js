...
export async function fetchList() {
  try {
    const res = await safeFetch("https://extrabutterny.com/collections/footwear.json");
    return res.products || res.items || res.results || res.objects || [];
  } catch (err) {
    console.error("extraButter fetch failed:", err.message);
    return [];
  }
}
...
export function normalize(p) {
  return {
    id: "extraButter-" + (p.id || p.handle || Date.now()),
    retailerId: "extraButter",
    retailerName: "Extra Butter",
    ...
    url: "https://extrabutterny.com/collections/footwear.json/products/" + (p.handle || p.slug || p.id || ""),
    region: "US"
  };
}
