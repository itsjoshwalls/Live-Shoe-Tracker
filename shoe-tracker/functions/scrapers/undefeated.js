...
export async function fetchList() {
  try {
    const res = await safeFetch("https://undefeated.com/collections/footwear.json");
    return res.products || res.items || res.results || res.objects || [];
  } catch (err) {
    console.error("undefeated fetch failed:", err.message);
    return [];
  }
}
...
export function normalize(p) {
  return {
    id: "undefeated-" + (p.id || p.handle || Date.now()),
    retailerId: "undefeated",
    retailerName: "Undefeated",
    ...
    url: "https://undefeated.com/collections/footwear.json/products/" + (p.handle || p.slug || p.id || ""),
    region: "US"
  };
}
