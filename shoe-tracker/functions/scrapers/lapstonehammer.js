...
export async function fetchList() {
  try {
    const res = await safeFetch("https://lapstoneandhammer.com/collections/footwear.json");
    return res.products || res.items || res.results || res.objects || [];
  } catch (err) {
    console.error("lapstonehammer fetch failed:", err.message);
    return [];
  }
}
...
export function normalize(p) {
  return {
    id: "lapstonehammer-" + (p.id || p.handle || Date.now()),
    retailerId: "lapstonehammer",
    retailerName: "Lapstone & Hammer",
    ...
    url: "https://lapstoneandhammer.com/collections/footwear.json/products/" + (p.handle || p.slug || p.id || ""),
    region: "US"
  };
}
