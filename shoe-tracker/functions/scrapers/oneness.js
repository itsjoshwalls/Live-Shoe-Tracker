...
export async function fetchList() {
  try {
    const res = await safeFetch("https://www.onenessboutique.com/collections/footwear.json");
    return res.products || res.items || res.results || res.objects || [];
  } catch (err) {
    console.error("oneness fetch failed:", err.message);
    return [];
  }
}
...
export function normalize(p) {
  return {
    id: "oneness-" + (p.id || p.handle || Date.now()),
    retailerId: "oneness",
    retailerName: "Oneness",
    ...
    url: "https://www.onenessboutique.com/collections/footwear.json/products/" + (p.handle || p.slug || p.id || ""),
    region: "US"
  };
}
