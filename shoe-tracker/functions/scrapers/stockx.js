...
export async function fetchList() {
  try {
    const res = await safeFetch("https://stockx.com/api/browse");
    return res.products || res.items || res.results || res.objects || [];
  } catch (err) {
    console.error("stockx fetch failed:", err.message);
    return [];
  }
}
...
export function normalize(p) {
  return {
    id: "stockx-" + (p.id || p.handle || Date.now()),
    retailerId: "stockx",
    retailerName: "StockX",
    ...
    url: "https://stockx.com/api/browse/products/" + (p.handle || p.slug || p.id || ""),
    region: "US"
  };
}
