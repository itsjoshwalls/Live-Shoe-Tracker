...
export async function fetchList() {
  try {
    const res = await safeFetch("https://kith.com/collections/footwear.json");
    return res.products || res.items || res.results || res.objects || [];
  } catch (err) {
    console.error("kith fetch failed:", err.message);
    return [];
  }
}
...
export function normalize(p) {
  return {
    id: "kith-" + (p.id || p.handle || Date.now()),
    retailerId: "kith",
    retailerName: "Kith",
    ...
    url: "https://kith.com/collections/footwear.json/products/" + (p.handle || p.slug || p.id || ""),
    region: "US"
  };
}
