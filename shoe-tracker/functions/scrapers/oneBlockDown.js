...
export async function fetchList() {
  try {
    const res = await safeFetch("https://www.oneblockdown.it/collections/footwear.json");
    return res.products || res.items || res.results || res.objects || [];
  } catch (err) {
    console.error("oneBlockDown fetch failed:", err.message);
    return [];
  }
}
...
export function normalize(p) {
  return {
    id: "oneBlockDown-" + (p.id || p.handle || Date.now()),
    retailerId: "oneBlockDown",
    retailerName: "One Block Down",
    ...
    url: "https://www.oneblockdown.it/collections/footwear.json/products/" + (p.handle || p.slug || p.id || ""),
    region: "US"
  };
}
