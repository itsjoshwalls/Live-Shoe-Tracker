...
export async function fetchList() {
  try {
    const res = await safeFetch("https://www.kickz.com/api/products");
    return res.products || res.items || res.results || res.objects || [];
  } catch (err) {
    console.error("kickz fetch failed:", err.message);
    return [];
  }
}
...
export function normalize(p) {
  return {
    id: "kickz-" + (p.id || p.handle || Date.now()),
    retailerId: "kickz",
    retailerName: "Kickz",
    ...
    url: "https://www.kickz.com/api/products/products/" + (p.handle || p.slug || p.id || ""),
    region: "US"
  };
}
