...
export default class NotreScraper extends BaseScraper {
  constructor() {
    super({ name: 'notreshop', baseUrl: 'https://notre-shop.com' });
  }
  ...
  async fetchReleases() {
    ...
    for (const collection of collections) {
      try {
          ...
          for (const product of data.products) {
            if (this.isSneaker(product)) {
              releases.push(this.normalizeRelease({
                ...
                locations: [{ name: 'Chicago', type: 'flagship' }],
                raw: product
              }));
            }
          }
        ...
        console.error(`[notreshop] Collection ${collection} failed:`, err.message);
      }
    }

    console.log(`[notreshop] Fetched ${releases.length} releases`);
    return releases;
  }

  isSneaker(product) {
    const text = `${product.title} ${product.product_type} ${product.vendor || ''}`.toLowerCase();
    return /sneaker|shoe|trainer|runner|boot|jordan|nike|adidas|yeezy|new balance|asics|salomon/i.test(text);
  }
}
