...
export default class SaintAlfredScraper extends BaseScraper {
  constructor() {
    super({ name: 'saintalfred', baseUrl: 'https://saintalfred.com' });
  }

  async fetchReleases() {
    const releases = [];
    const collections = ['footwear', 'new-arrivals'];
    ...
    for (const collection of collections) {
      try {
        ...
        console.error(`[saintalfred] Collection ${collection} failed:`, err.message);
      }
    }

    console.log(`[saintalfred] Fetched ${releases.length} releases`);
    return releases;
  }
